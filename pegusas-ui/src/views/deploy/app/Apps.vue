  ...
  <el-row class="panel">
      ...
      <div class="List-title List-titleText" style="margin: 10px">
        <el-dropdown>
          ...
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item @click.native="changeConfig(true)">标准化</el-dropdown-item>
            <el-dropdown-item @click.native="changeConfig(false)">非标准化</el-dropdown-item>
          </el-dropdown-menu>
        </el-dropdown>
      </div>
    </el-col>
    ...
    <el-col :span="24" class="list">
      <el-table :data="apps.results">
        ...
        <el-table-column prop="time" label="上线时间" min-width="100"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    setAppBacth (row) {
      this.$router.push({ name: 'AppBatch', params: { batchId: row.name, team: row.team } })
    },
    changeConfig (type) {
      this.standard = type
      if (type) {
        this.standardMsg = '标椎化'
        this.queryStandardApps()
      } else {
        this.standardMsg = '非标椎化'
        this.queryUnStandardApps()
      }
    }
  },
